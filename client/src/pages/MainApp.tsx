...
export default function MainApp({
  ...
  const [sharePreviewDialog, setSharePreviewDialog] = useState<{
    open: boolean;
    activity: ActivityType | null;
  }>({ open: false, activity: null });
  ...
  const [journalActivityContext, setJournalActivityContext] = useState<{
    activityId: string;
    title: string;
  } | null>(null);
  const [postActivityPrompt, setPostActivityPrompt] = useState<{
    open: boolean;
    activity: ActivityType | null;
  }>({ open: false, activity: null });
  const [promptedActivities, setPromptedActivities] = useState<Set<string>>(
    () => {
      try {
        ...
        console.error(
          "Failed to parse promptedActivities from localStorage:",
          error,
        );
  ...
  const [upgradeTrigger, setUpgradeTrigger] = useState<
    "planLimit" | "favorites" | "export" | "insights"
  >("planLimit");
  ...
  const [shareActivityDialog, setShareActivityDialog] = useState<{
    open: boolean;
    activityId: string;
  ...
  const handlePlanCommand = (text: string): boolean => {
    ...
    if (
      /\b(journal\s*this|save\s*(to\s*)?(my\s*)?journal|add\s*(to\s*)?(my\s*)?journal)\b/.test(
        lowerText,
      )
    ) {
    ...
    if (
      /\b(fit\s*(my\s*)?budget|match\s*budget|budget\s*filter|filter\s*(by\s*)?budget|within\s*budget)\b/.test(
        lowerText,
      )
    ) {
      ...
      toast({
        title: state.matchBudgetFilter
          ? "Budget Filter Disabled"
          : "Budget Filter Enabled",
        description: state.matchBudgetFilter
          ? "Showing all alternatives"
          : "Filtering alternatives to match your budget",
      });
      return true;
    }
    ...
    if (
      /\b(show\s*alternatives|see\s*alternatives|see\s*other\s*options|view\s*alternatives|expand\s*alternatives)\b/.test(
        lowerText,
      )
    ) {
    ...
    if (
      /\b(hide\s*alternatives|close\s*alternatives|collapse\s*alternatives)\b/.test(
        lowerText,
      )
    ) {
  ...
  const getActivityText = (activity: {
    ...
    switch (activity.activityType) {
      ...
      case "task_added":
        return (
          <>
            <span className="font-medium">{userName}</span> added new task{" "}
            <span className="font-medium">"{taskName}"</span>
          </>
        );
      case "activity_shared":
  ...
  const handleArchiveActivity = useMutation({
    ...
    onError: (error: any) => {
      toast({
        ...
        description:
          error?.message || "Failed to archive activity. Please try again.",
      });
    },
  });
  ...
  useEffect(() => {
    ...
    if (authStatus === "error") {
      if (provider === "facebook" && reason === "app_not_active") {
        toast({
          ...
          description:
            "Facebook authentication is currently in development mode. Please use Google or Email to sign in.",
          variant: "destructive",
          duration: 8000,
        });
      } else {
    ...
    if (isNewUser && userId) {
      toast({
        title: "Welcome! 🎉",
        description:
          "Your account has been created. Complete your profile to get personalized recommendations!",
        duration: 5000,
      });
        ...
        <div className="max-w-md text-center space-y-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground mb-4">
              {feature} and other features require a free account. Sign in to
              unlock unlimited activities, progress tracking, and more!
            </p>
          </div>
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Free users: 1 activity • Signed in: Unlimited activities +
              features
            </p>
          </div>
        </div>
  ...
  const {
    data: chatImports = [],
    isLoading: chatImportsLoading,
    refetch: refetchChatImports,
  } = useQuery<ChatImport[]>({
  ...
  const {
    data: activityTasks,
    isLoading: activityTasksLoading,
    error: activityTasksError,
  } = useQuery<Task[]>({
  ...
  const processGoalMutation = useMutation({
    ...
    onSuccess: async (data: any, variables: string) => {
      ...
      console.log(
        "🔄 Plan refinement - preserving activityId:",
        preservedActivityId,
      );
      ...
      console.log("📋 New plan output:", {
        ...newPlanOutput,
        tasks: `${newPlanOutput.tasks.length} tasks`,
      });
  ...
  const completeTaskMutation = useMutation({
    ...
    onError: (error: any, taskId: string, context: any) => {
      ...
      const errorMessage =
        error?.response?.error || error.message || "Failed to complete task";
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive",
  ...
  const skipTaskMutation = useMutation({
    mutationFn: async (taskId: string) => {
      const response = await apiRequest("POST", `/api/tasks/${taskId}/skip`, {
        reason: "User skipped via swipe",
      });
      return response.json();
    },
    onMutate: async (taskId: string) => {
    ...
    onError: (error: any, taskId: string, context: any) => {
      ...
      const errorMessage =
        error?.response?.error || error.message || "Failed to skip task";
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive",
  ...
  const snoozeTaskMutation = useMutation({
    mutationFn: async ({
      ...
      const response = await apiRequest("POST", `/api/tasks/${taskId}/snooze`, {
        hours,
      });
      return response.json();
    },
    ...
    onError: (
      error: any,
      { taskId }: { taskId: string; hours: number },
      context: any,
    ) => {
      // Rollback optimistic update
      if (context?.previousTasks) {
      ...
      const errorMessage =
        error?.response?.error || error.message || "Failed to snooze task";
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive",
  ...
  const importChatMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      toast({
        ...
        description:
          data.message ||
          `Created ${data.tasks?.length || 0} accountability tasks!`,
      });
      setChatText("");
      setChatTitle("");
    ...
    onError: (error: any) => {
      const errorMessage =
        error?.response?.error ||
        error.message ||
        "Failed to import chat history. Please try again.";
      toast({
        title: "Import Failed",
        description: errorMessage,
        variant: "destructive",
  ...
  const handleChatImport = () => {
    ...
    try {
      ...
      toast({
        title: "Parsing Error",
        description:
          "Could not parse chat format. Please check your chat text.",
        variant: "destructive",
      });
    }
  };
  ...
  const completeTutorialMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isDemo) {
        ...
        console.log(
          "[TUTORIAL] Demo user tutorial marked as completed (localStorage)",
        );
      ...
      const response = await fetch(
        `/api/users/${(user as any).id}/complete-tutorial`,
        {
          method: "POST",
  ...
  const createActivityMutation = useMutation({
    ...
    onSuccess: async (data: any) => {
      ...
      const updatedTasks =
        queryClient.getQueryData<Task[]>(["/api/tasks"]) || [];
      ...
      setCurrentPlanOutput((prev) => {
        ...
        const tasksWithIds = prev.tasks.map((previewTask, index) => {
          ...
          const createdTask = updatedTasks.find(
            (t) =>
        ...
        console.log(
          "📝 Updated currentPlanOutput with activityId and real task IDs:",
          updated.activityId,
          tasksWithIds.map((t) => t.id),
        );
        return updated;
      });
    ...
    onError: async (error: any) => {
      ...
      if (errorData?.requiresAuth) {
        toast({
          ...
          description:
            errorData.message ||
            "Sign in to create unlimited activities and access all features",
          action: (
            <Button
              size="sm"
              variant="default"
  ...
  useEffect(() => {
    if (activities.length > 0 && completedActivities.size > 0) {
      activities.forEach((activity) => {
        ...
        if (
          activity.progressPercent === 100 &&
          !Array.from(completedActivities).includes(activity.id)
        ) {
  ...
  const handleQuickNote = (activityId: string, title: string) => {
    ...
    localStorage.setItem(
      "journalmate_prompted_activities",
  ...
  return (
    <div className="flex flex-col h-screen bg-background overflow-hidden">
                ...
                <TooltipTrigger asChild>
                  <div className="relative">
                    {(user as any)?.id === "demo-user" &&
                      !localStorage.getItem("demo-tutorial-completed") && (
                    ...
                    <Button
                      ...
                      aria-label={
                        isAuthenticated && (user as any)?.id !== "demo-user"
                          ? "Help & Tutorial"
                          : "Help & Live Demo"
                      }
                    >
            ...
            >
                  ...
                  <div className="max-w-4xl mx-auto mb-6 px-4">
                    <div className="bg-primary/10 border border-primary/20 rounded-lg p-4">
                      ...
                      <p className="text-sm text-muted-foreground mb-3">
                        Tell me what you'd like to change - modify tasks, update
                        description, adjust priority, or anything else.
                      </p>
                      <div className="text-xs text-muted-foreground space-y-1">
                        <p>
                          <strong>Category:</strong> {editingActivity.category}
                        </p>
                        {editingActivity.planSummary && (
                          <p>
                    ...
                    <div className="max-w-4xl mx-auto space-y-6">
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Badge
                                  variant="secondary"
                                  className="text-xs"
                                  data-testid="badge-date-planner-beta"
                                >
                                  Beta
                                </Badge>
                              </TooltipTrigger>
                      ...
                      <div className="max-w-2xl mx-auto">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {[
                            {
                              text: "Devise a workout plan",
                              theme: "Health & Fitness",
                              Icon: Dumbbell,
                            },
                            {
                              text: "Focus on work productivity",
                              theme: "Work Focus",
                              Icon: Briefcase,
                            },
                            {
                              text: "Trade stocks using AI insights",
                              theme: "Investment",
                              Icon: TrendingUp,
                            },
                            {
                              text: "Create morning devotion plan",
                              theme: "Spiritual",
                              Icon: BookOpen,
                            },
                            {
                              text: "Plan perfect date night",
                              theme: "Romance",
                              Icon: Heart,
                            },
                            {
                              text: "Explore hiking adventures",
                              theme: "Adventure",
                              Icon: Mountain,
                            },
                          ].map((example, index) => {
                            ...
                            return (
                              <Button
                                ...
                                size="lg"
                                onClick={() =>
                                  processGoalMutation.mutate(example.text)
                                }
                              ...
                              >
                                ...
                                <span className="text-sm leading-tight overflow-hidden text-ellipsis">
                                  {example.text}
                                </span>
                              </Button>
                            );
                ...
                >
                  <DialogContent className="max-w-[95vw] sm:max-w-4xl">
                    <DialogHeader backLabel="Back to Planning">
                      ...
                      <DialogDescription>
                        Let us help you find the perfect spots for your date
                    ...
                    <ClaudePlanOutput
                      ...
                      tasks={currentPlanOutput.tasks.map((task) => {
                        ...
                        return {
                          ...
                          priority:
                            ...
                            "medium",
                          completed:
                            actualTask?.completed ?? task.completed ?? false,
                      ...
                      onSetAsTheme={() => {
                        ...
                        toast({
                          title: "Theme Set!",
                          description:
                            "This plan is now your focus theme for today. (Feature coming soon!)",
                        });
                      }}
                      ...
                      backdrop={
                        activities?.find(
                          (a) => a.id === currentPlanOutput.activityId,
                        )?.backdrop ?? undefined
                      }
                      showConfetti={true}
                    />
              ...
              <TabsContent value="activities" className="space-y-6 pb-20">
                <div className="text-center mb-6 px-4">
                  ...
                  <p className="text-sm sm:text-base text-muted-foreground">
                    Shareable activities with progress tracking and social
                    features. Click an activity to view its tasks.
                  </p>
                </div>
                  ...
                  <div className="space-y-4 max-w-4xl mx-auto">
                    {activities.map((activity) => {
                      ...
                      const completedTasks =
                        activityWithProgress.completedTasks || 0;
                      const totalTasks = activityWithProgress.totalTasks || 0;
                      const progressPercent =
                        activityWithProgress.progressPercent || 0;
                                  ...
                                  onClick={(e) => {
                                    ...
                                    if (
                                      ...
                                      toast({
                                        ...
                                        description:
                                          "Please sign in to share your activities with others",
                                        action: (
                                          <Button
                                            size="sm"
                                            variant="default"
                                    ...
                                    if (!activity.isPublic) {
                                      toast({
                                        ...
                                        description:
                                          "Make the activity public first before sharing.",
                                        variant: "default",
                                      });
                                      return;
                                    }
                                ...
                                <Button
                                  ...
                                  onClick={async (e) => {
                                    ...
                                    try {
                                      // Toggle like status
                                      const response = await fetch(
                                        `/api/activities/${activity.id}/feedback`,
                                        {
                                          method: "POST",
                                          headers: {
                                            "Content-Type": "application/json",
                    ...
                    {(() => {
                      ...
                      if (selectedActivityId && activityTasksLoading) {
                        return (
                          <div className="text-center py-8">
                            <p className="text-muted-foreground">
                              Loading tasks for selected activity...
                            </p>
                          </div>
                        );
                      }
                      ...
                      if (selectedActivityId && activityTasksError) {
                        return (
                          <div className="text-center py-8">
                            <p className="text-destructive">
                              Failed to load activity tasks. Please try again.
                            </p>
                          </div>
                        );
                      }
                  ...
                  <div className="max-w-4xl mx-auto space-y-8">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-muted-foreground max-w-lg mx-auto">
                        Share or paste content from anywhere. We'll extract it,
                        create an actionable plan, and add it to your journal.
                      </p>
                    </div>

                    {/* Supported Sources */}
                    <Card className="p-6">
                      <p className="text-xs font-semibold text-muted-foreground text-center mb-4 tracking-wider">
                        SUPPORTED SOURCES
                      </p>
                      ...
                      <div className="mt-6 space-y-3">
                          ...
                          onClick={async () => {
                            try {
                              ...
                              toast({
                                ...
                                description:
                                  "Please paste your content manually in the text area below.",
                                variant: "destructive",
                              });
                            }
                          }}
                        ...
                        <p className="text-xs text-muted-foreground text-center">
                          On mobile, use the share button in any app to send
                          content directly to JournalMate
                        </p>
                      </div>
                    </Card>
                    ...
                    <Card className="p-6">
                      ...
                      <div className="space-y-4">
                        <div className="flex gap-3 items-start p-3 bg-gradient-to-r from-pink-50 to-purple-50 dark:from-pink-900/20 dark:to-purple-900/20 rounded-lg">
                          ...
                          <div>
                            <p className="font-medium text-sm">
                              Found a workout routine on Instagram?
                            </p>
                            <p className="text-xs text-muted-foreground">
                              Tap Share → Select JournalMate → Get your
                        ...
                        <div className="flex gap-3 items-start p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg">
                          ...
                          <div>
                            <p className="font-medium text-sm">
                              Had a trip planning chat with ChatGPT?
                            </p>
                            <p className="text-xs text-muted-foreground">
                              Copy the conversation → Paste here → Get
                              actionable travel tasks!
                            </p>
                          </div>
                        </div>
                        <div className="flex gap-3 items-start p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Tap Share → JournalMate → We extract ingredients
                              and steps!
                            </p>
                          </div>
                        </div>
                          ...
                          {chatImports.slice(0, 5).map((chatImport) => (
                            ...
                            >
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {chatImport.extractedGoals?.length || 0} goals
                                  extracted •{" "}
                              ...
                              <div className="flex items-center gap-2 ml-4">
                                ...
                                <Button
                                  ...
                                  variant="ghost"
                                  className="h-7 w-7"
                                  data-testid={`button-view-import-${chatImport.id}`}
                                >
                  ...
                  <div className="max-w-4xl mx-auto px-4">
                    ...
                    <div className="grid gap-6 md:grid-cols-2 mb-8">
                      ...
                      <Card className="p-6">
                        <div className="text-center">
                          ...
                          <p className="text-muted-foreground mb-4">
                            Start a new group for shared activities and
                      ...
                      {groupActivitiesLoading ? (
                        ...
                        <Card className="p-8">
                          <div className="text-center text-muted-foreground">
                            ...
                            <p>
                              No recent activity. Group activities will appear
                              here when members complete tasks or make changes.
                            </p>
                          </div>
                        </Card>
                      )}
                    ...
                    <Card className="p-6 bg-gradient-to-r from-purple-500/5 to-emerald-500/5 border-dashed">
                      <div className="text-center mb-4">
                        ...
                        <p className="text-sm text-muted-foreground mb-4">
                          Browse plans created by others, get inspired, and use
                          them for your own goals. Join thousands planning
                          together!
                        </p>
                        <Link href="/discover">
                          <Button
                ...
                <div className="max-w-4xl mx-auto">
                  ...
                  <div className="text-center mb-12">
                    <div className="inline-flex items-center justify-center w-32 h-32 mb-6">
                      <img
                        src="/journalmate-logo-transparent.png"
                        alt="AI Planner - Smart Goal Tracker and AI Journal for Life Planning"
                    ...
                    <p className="text-xl sm:text-2xl text-muted-foreground max-w-3xl mx-auto leading-relaxed mb-4">
                      Watch your dreams become your reality with JournalMate's
                      rhythm-aware planning engine
                    </p>
                    <p className="text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed">
                      Unlike traditional journals, JournalMate combines{" "}
                      ...
                      , and{" "}
                      <span className="font-semibold text-foreground">
                        rhythm-aware journaling
                      </span>{" "}
                      to help you PLAN first, then REFLECT.
                    </p>
                    ...
                    <div className="mt-8 max-w-3xl mx-auto">
                      ...
                      <p className="text-xs sm:text-sm text-muted-foreground mt-4">
                        Your complete planning-to-reflection journey, powered by
                        AI that learns your rhythm
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="text-center mb-8">
                    ...
                    <p className="text-sm text-muted-foreground mt-3">
                      Start with Quick Plan (5 questions) or Smart Plan
                      (comprehensive with real-time web research)
                    </p>
                  </div>
                  ...
                  <div className="grid gap-6 md:grid-cols-3 mb-12">
                    ...
                    <div
                      ...
                      onClick={() =>
                        setExpandedFeature(
                          expandedFeature === "planning" ? null : "planning",
                        )
                      }
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground">
                        AI that learns your rhythm and adapts plans in real-time
                          ...
                          <div className="space-y-3 text-sm">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-muted-foreground">
                                  5 essential questions in 2 batches with
                                  targeted web research for flight prices,
                                  hotels, and weather
                                </p>
                              </div>
                            </div>
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-muted-foreground">
                                  10 comprehensive questions in 3 batches with
                                  mandatory real-time enrichment: resort
                                  pricing, restaurant details, flight
                                  comparisons, 7-day forecasts
                                </p>
                              </div>
                            </div>
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-muted-foreground">
                                  Dynamic emojis based on destination,
                                  intelligent routing explanations (e.g., "Fly
                                  to Cancun instead of Tulum - TQO has limited
                                  service"), and budget-conscious
                                  recommendations
                                </p>
                              </div>
                            </div>
                            <div className="flex items-center gap-2 text-xs text-purple-600 dark:text-purple-400 pt-2">
                              ...
                              <span>
                                Import from ChatGPT, Claude, and other AI
                                assistants
                              </span>
                            </div>
                          </div>
                    ...
                    <div
                      ...
                      onClick={() =>
                        setExpandedFeature(
                          expandedFeature === "execution" ? null : "execution",
                        )
                      }
                    ...
                    >
                      ...
                      <h3 className="text-lg font-semibold mb-2">
                        Activity Execution & Progress Tracking
                      </h3>
                      <p className="text-sm text-muted-foreground">
                        Execute your plans with swipeable task cards, real-time
                        analytics, and celebratory milestones
                      </p>
                        ...
                        <div className="mt-4 p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-200 dark:border-emerald-700 text-left">
                          <div className="space-y-3 text-sm">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <p className="font-medium text-foreground">
                                  Swipeable Task Interface
                                </p>
                                <p className="text-muted-foreground">
                                  Swipe right to complete, left to skip. Every
                                  completion triggers celebration animations
                                  with confetti effects
                                </p>
                              </div>
                            </div>
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-muted-foreground">
                                  Track completion rates, streaks, productivity
                                  patterns with visual analytics that adapt to
                                  your workflow
                                </p>
                              </div>
                            </div>
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <p className="font-medium text-foreground">
                                  YouTube-Style Feedback
                                </p>
                                <p className="text-muted-foreground">
                                  Thumbs up/down on tasks to help AI learn what
                                  works for you and refine future plans
                                </p>
                              </div>
                            </div>
                          </div>
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground">
                        Close the loop with intelligent reflection that learns
                        your patterns and enriches future planning
                      </p>
                          ...
                          <div className="space-y-3 text-sm">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-muted-foreground">
                                  Type minimal text with keywords (@restaurants,
                                  @travel, @music) + upload photos/videos. AI
                                  auto-detects category and enriches entries
                                </p>
                              </div>
                            </div>
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-muted-foreground">
                                  Restaurants, Travel, Music, Movies, Books,
                                  Products, Workouts, Personal Achievements,
                                  Daily Reflections with AI-organized galleries
                                </p>
                              </div>
                            </div>
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <p className="font-medium text-foreground">
                                  Feeds Future Planning
                                </p>
                                <p className="text-muted-foreground">
                                  AI learns from your reflections to personalize
                                  future plan recommendations and task
                                  suggestions
                                </p>
                              </div>
                            </div>
                  ...
                  <div
                    ...
                    onClick={() =>
                      setExpandedFeature(
                        expandedFeature === "sharing" ? null : "sharing",
                      )
                    }
                  ...
                  >
                    <div className="flex items-center justify-center gap-4 mb-6">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Share your success, inspire others, discover community
                          plans
                        </p>
                      </div>
                    </div>
                        ...
                        <div className="space-y-4 text-sm">
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-muted-foreground">
                                Import your phone contacts safely with privacy
                                protection and share via SMS, email, or direct
                                links
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-muted-foreground">
                                Auto-generated invitation messages for sharing
                                your goals, journal entries, and collaborative
                                planning sessions
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-muted-foreground">
                                Perfect for couples goals, family planning,
                                group trips, and accountability partnerships
                                with live updates
                              </p>
                            </div>
                          </div>
                  ...
                  <div className="grid gap-4 md:grid-cols-2 mb-12">
                    <div className="p-4 bg-muted/50 rounded-xl border">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Track goal completion rates, daily streaks, habit
                        formation, and productivity insights with visual charts
                      </p>
                    </div>
                    ...
                    <div className="p-4 bg-muted/50 rounded-xl border">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Import and sync plans from ChatGPT, Claude, and other AI
                        chatbots directly into your task manager
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="text-center p-6 bg-muted/30 rounded-xl border">
                    <p className="text-lg font-semibold mb-2">
                      Founded by Dennis Tanaruno
                    </p>
                    <p className="text-muted-foreground mb-4">
                      Built for those who want to live with intention and
                      momentum
                    </p>
                    <Button asChild variant="outline" size="sm">
                      <a
                        href="https://www.linkedin.com/in/dennis-tanaruno"
                        target="_blank"
                    ...
                    <div className="mt-6 pt-4 border-t border-muted">
                      <p className="text-xs text-muted-foreground">
                        © {new Date().getFullYear()} JournalMate. All rights
                        reserved.
                      </p>
                      <p className="text-xs text-muted-foreground mt-1">
                        The JournalMate name, logo, design, and all related
                        intellectual property are protected by copyright and
                        trademark laws.
                      </p>
                    </div>
                  </div>
      ...
      <Dialog open={showContacts} onOpenChange={onShowContacts}>
        <DialogContent
          className="max-w-[95vw] sm:max-w-6xl h-[90vh] flex flex-col"
      ...
      <Dialog open={showChatHistory} onOpenChange={onShowChatHistory}>
        <DialogContent
          className="max-w-[95vw] sm:max-w-6xl h-[90vh] flex flex-col"
          data-testid="modal-chat-history"
        >
          <DialogHeader backLabel="Back to Home">
      ...
      <Dialog open={showRecentGoals} onOpenChange={onShowRecentGoals}>
        <DialogContent
          className="max-w-[95vw] sm:max-w-6xl h-[90vh] flex flex-col"
          data-testid="modal-recent-goals"
        >
          <DialogHeader backLabel="Back to Home">
      ...
      <Dialog open={showProgressReport} onOpenChange={onShowProgressReport}>
        <DialogContent
          className="max-w-[95vw] sm:max-w-6xl h-[90vh] flex flex-col"
          data-testid="modal-progress-report"
        >
          <DialogHeader backLabel="Back to Home">
            <DialogTitle>Progress Report</DialogTitle>
            <DialogDescription>
              Comprehensive analytics, milestones, and insights about your
              achievements
            </DialogDescription>
          </DialogHeader>
          <div className="flex-1 overflow-y-auto min-h-0">
      ...
      >
        <DialogContent
          ...
          data-testid="modal-journal-mode"
        >
      ...
      {postActivityPrompt.activity && (
        <PostActivityPrompt
          ...
          onOpenChange={(open) =>
            setPostActivityPrompt({ ...postActivityPrompt, open })
          }
        />
      )}
      ...
      >
        <AlertDialogContent data-testid="dialog-delete-activity">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete "{deleteDialog.activity?.title}"?
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              disabled={handleDeleteActivity.isPending}
            >
              {handleDeleteActivity.isPending
                ? "Deleting..."
                : "Delete Activity"}
            </AlertDialogAction>
          </AlertDialogFooter>
      ...
      {sharePreviewDialog.activity && (
        <SharePreviewDialog
          ...
          onOpenChange={(open) =>
            setSharePreviewDialog({ open, activity: null })
          }
          ...
          onConfirmShare={async (shareData) => {
            ...
            try {
              ...
              const { getContextualEmoji } = await import(
                "@/lib/shareCardTemplates"
              );
              ...
              if (!shareUrl) {
                console.warn(
                  "No share link generated, but activity has been saved/shared successfully",
                );
                toast({
                  title: "Success!",
                  description:
                    "Your activity has been saved and shared. You can share it later from the activity details.",
                });
                return;
              }

              const displayTitle =
                shareData.shareTitle || activity.planSummary || activity.title;
              const contextualEmoji = getContextualEmoji(
                activity.title,
                activity.category,
              );
              ...
              if (isNative() && shareData.shareCardImageFile) {
                try {
                  ...
                  console.warn(
                    "Capacitor Share failed, trying Web Share API:",
                    shareError,
                  );
                }
              } else if (isNative()) {
              ...
              if (
                ...
                try {
                  if (
                    navigator.canShare &&
                    navigator.canShare({
                      files: [shareData.shareCardImageFile],
                    })
                  ) {
                  ...
                  console.warn(
                    "Web Share API with files failed, trying text-only share:",
                    shareError,
                  );
                }
              }
              ...
              if (navigator.share && !shareSuccessful) {
                ...
                try {
                  ...
                  console.warn(
                    "Navigator.share failed, trying clipboard:",
                    shareError,
                  );
                }
              }
              ...
              if (!shareSuccessful) {
                ...
                try {
                  ...
                  toast({
                    ...
                    description:
                      "Share link has been copied. Paste it anywhere to share!",
                  });
                  shareSuccessful = true;
              ...
              if (
                ...
                toast({
                  ...
                  description:
                    "Please allow clipboard access to copy the share link.",
                  variant: "destructive",
                });
                return;
              }
