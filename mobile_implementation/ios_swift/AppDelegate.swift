...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate, MessagingDelegate {
    ...
    func application(_ app: UIApplication, open url: URL, options: [UIApplication.OpenURLOptionsKey: Any] = [:]) -> Bool {
        // Handle incoming share links (journalmate://share or https://journalmate.ai/share)
    ...
    func application(_ application: UIApplication, continue userActivity: NSUserActivity, restorationHandler: @escaping ([UIUserActivityRestoring]?) -> Void) -> Bool {
        // Handle Universal Link shares (https://journalmate.ai/share/...)
        if let url = userActivity.webpageURL, handleShareUrl(url) {
            return true
        }
    ...
    private func handleShareUrl(_ url: URL) -> Bool {
        // Avoid intercepting OAuth callbacks
        if url.scheme == "journalmate", url.host == "auth" {
            return false
        }

        let absolute = url.absoluteString.lowercased()
        let isShareLink = absolute.contains("/share") || url.host == "share"
        guard isShareLink else {
            return false
        }

        guard let payload = buildSharePayload(from: url) else {
            return false
        }

        SharePlugin.setPendingShare(payload)
        print("[JournalMate] Share payload captured: \(payload)")
        return true
    }

    private func buildSharePayload(from url: URL) -> String? {
        ...
        if urlString == nil {
            // If the link itself is a shared URL, use it
            if url.scheme?.hasPrefix("http") == true {
                urlString = url.absoluteString
            }
        }

        if textString == nil, let urlString {
