...
public class BackgroundServicePlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    private let foregroundServiceKey = "foreground_service_enabled"
    private let foregroundNotificationId = "journalmate_foreground_service"
    ...
    @objc func startForegroundService(_ call: CAPPluginCall) {
        prefs.set(true, forKey: foregroundServiceKey)
        showOrUpdateForegroundNotification()
        print("[BackgroundService] startForegroundService - showing persistent status notification")
        call.resolve(["success": true])
    }

    @objc func stopForegroundService(_ call: CAPPluginCall) {
        prefs.set(false, forKey: foregroundServiceKey)
        UNUserNotificationCenter.current().removePendingNotificationRequests(withIdentifiers: [foregroundNotificationId])
        UNUserNotificationCenter.current().removeDeliveredNotifications(withIdentifiers: [foregroundNotificationId])
        print("[BackgroundService] stopForegroundService - cleared status notification")
        call.resolve(["success": true])
    }
    ...
    @objc func updateProgress(_ call: CAPPluginCall) {
        ...
        if prefs.bool(forKey: foregroundServiceKey) {
            showOrUpdateForegroundNotification()
        }
        call.resolve(["success": true])
    }
    ...
    // MARK: - Foreground Status Notification

    private func showOrUpdateForegroundNotification() {
        let completed = prefs.integer(forKey: "progress_completed_tasks")
        let total = prefs.integer(forKey: "progress_total_tasks")
        let streak = prefs.integer(forKey: "progress_streak")
        let nextTaskTitle = prefs.string(forKey: "progress_next_task_title") ?? ""

        let title = "JournalMate Active"
        var body = "Today's progress: \(completed)/\(total) tasks • Streak \(streak)"
        if !nextTaskTitle.isEmpty {
            body += " • Next: \(nextTaskTitle)"
        }
        ...
        content.body = body
        content.sound = nil
        content.userInfo = ["notificationId": foregroundNotificationId, "route": "/app?tab=tasks"]

        // Replace the existing notification with the same identifier
        let trigger = UNTimeIntervalNotificationTrigger(timeInterval: 1, repeats: false)
        let request = UNNotificationRequest(identifier: foregroundNotificationId, content: content, trigger: trigger)

        UNUserNotificationCenter.current().add(request) { error in
            if let error = error {
                print("[BackgroundService] Foreground notification failed: \(error.localizedDescription)")
            } else {
                print("[BackgroundService] Foreground status notification updated")
            }
        }
    }
