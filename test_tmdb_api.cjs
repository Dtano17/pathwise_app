...
async function testSearch(query, type = 'movie') {
  ...
  if (data.results && data.results.length > 0) {
    ...
    data.results.slice(0, 5).forEach((r, i) => {
      ...
      const year = type === 'movie' ? (r.release_date ? r.release_date.substring(0,4) : '?') : (r.first_air_date ? r.first_air_date.substring(0,4) : '?');
      const pop = r.popularity ? r.popularity.toFixed(1) : '0';
      console.log('  ' + (i+1) + '. "' + title + '" (' + year + ') - pop: ' + pop + ', votes: ' + r.vote_count + ', poster: ' + (r.poster_path ? 'YES' : 'NO'));
    });
...
async function getImages(id, type = 'movie') {
  ...
  console.log('  Images for ' + type + ' ' + id + ':');
  console.log('    Backdrops: ' + (data.backdrops ? data.backdrops.length : 0));
  console.log('    Posters: ' + (data.posters ? data.posters.length : 0));
...
async function main() {
  ...
  console.log('\n\n========== SPIDER-MAN TESTS ==========');
  await testSearch('Spider-Man: Brand New Day', 'movie');
