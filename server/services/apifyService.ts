...
class ApifyService {
  ...
  async extractInstagramReel(url: string): Promise<ApifyInstagramResult> {
    ...
    try {
      ...
      if (!items || items.length === 0) {
        console.log(`[APIFY] instagram-scraper returned no results, trying reel-scraper...`);
      ...
      if (item.carousel_media && item.carousel_media.length > 0) {
        const carouselItems = item.carousel_media.map((media: any) => ({
          ...
          url: media.video_url || media.display_url || media.image_versions2?.candidates?.[0]?.url
        })).filter((m: any) => m.url);
      ...
      if (error.response?.status === 402) {
        return { success: false, error: 'Apify credits exhausted. Please add more credits.' };
      }
  ...
  private async extractInstagramReelAlternative(url: string): Promise<ApifyInstagramResult> {
    try {
      ...
      console.error(`[APIFY] Alternative Instagram extraction also failed:`, error.message);
  ...
  async extractTikTokVideo(url: string): Promise<ApifyTikTokResult> {
    ...
    try {
      ...
      if (!items || items.length === 0) {
        console.log(`[APIFY] No results returned for TikTok, trying alternative actor...`);
      ...
      const videoUrl = item.videoUrl || item.webVideoUrl || item.videoMeta?.downloadAddr;

      if (item.imagePost && item.imagePost.images?.length > 0) {
        return {
          success: true,
          isSlideshow: true,
          slideshowImages: item.imagePost.images.map((img: any) => 
            img.imageURL?.urlList?.[0] || img.url
          ).filter(Boolean),
          caption: item.text,
  ...
  private async extractTikTokVideoAlternative(url: string): Promise<ApifyTikTokResult> {
    ...
    try {
      ...
      console.error(`[APIFY] Alternative TikTok extraction failed:`, error.message);
